...
```jsx
 <div>
  <UsageBar  series={[{name: 'a', value: 1},{name: 'a', value: 1},{name: 'a', value: 1}]}  isBulk  />
  <UsageBar  series={[{name: 'a', value: 1},{name: 'a', value: 1},{name: 'a', value: 1}]}  isBulk  withLenged />
 </div>
```
### 字节
```
...
### 水位线，阈值线
```
<UsageBar now={900000} max={7000000} waterLine={.6} />
```